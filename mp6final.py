...
class Scheduler:
    ...
    Sort key: (priority ASC, size ASC)
    """
    ...
    def build_queue(projects: dict[int, Project]) -> deque[int]:
        ...
        sorted_projects = sorted(
            active,
            key=lambda p: (p.priority, p.size)
        )
...
class App:
    ...
    def _create_schedule(self) -> None:
        ...
        # Preview table
        preview = sorted(active, key=lambda p: (p.priority, p.size))
        fmt = "  {:<10} {:<6}"
        print()
        separator()
        print(fmt.format("Priority", "Size"))
        separator()
        for p in preview:
            print(fmt.format(p.priority, p.size))
        separator()
    ...
    def _get_project(self) -> None:
        ...
        if confirm("Start and complete this project? (Y/N)"):
            self.queue.popleft()
            print(f"\n  Topmost project {p.id_number} removed from queue.")
            ...
            print(f"  Project {p.id_number} successfully moved to Completed Projects.")
            if self.queue:
                print("\n  Updated Queue:")
                self._print_queue()
            else:
